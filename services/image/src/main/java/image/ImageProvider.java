...
import image.setup.ImageIDFactory;
...
public enum ImageProvider {
  ...
  /**
   * Searches and returns the requested web interface images in the requested sizes.
   * If an image can not be found, the standard "not found" image is returned.
   * If an image is found in the incorrect size,
   * the largest size of this image will be scaled
   * and the scaled version will be moved to storage and returned.
   * @param images Map of product IDs and image sizes to search for.
   * @return Map between product IDs and base64 encoded image data as string.
   */
  public Map<String, String> getWebImages(Map<String, ImageSize> images) {
